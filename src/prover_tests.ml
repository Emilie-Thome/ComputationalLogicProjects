...
let test_string_of_ty1 = string_of_ty (Imp (Imp (TVar "A", TVar "B"), Imp (TVar "A", TVar "C")));;
let test_string_of_ty2 = string_of_ty (Imp (And (TVar "A", TVar "B"), TVar "A"));;
...
let test_string_of_tm1 = string_of_tm ( Abs ("f", Imp (TVar "A", TVar "B"),  Abs ("x", TVar "A", App(Var "f", Var "x"))));;
let test_string_of_tm2 = string_of_tm ( Abs ("f", Imp (And (TVar "A", TVar "B"), TVar "B"),  Abs ("t", And(TVar "A", TVar "B"), App(Var "f", Var "t"))));;
let test_string_of_tm3 = string_of_tm ( Abs ("t", And (TVar "A", TVar "B"), Fst (Var "t")));;
let test_string_of_tm4 = string_of_tm (Abs ("f", Imp (True, TVar "A"), App(Var "f", Unit)));;
let test_string_of_tm5 = string_of_tm (Right (TVar "A", Var "t"));;
...
let tyA = TVar "A";;
let tyAB = Imp (TVar "A", TVar "B");;
let tyBC = Imp (TVar "B", TVar "C");;
let tm2 = Abs ("f", tyAB, Abs ("g", tyBC, Abs ("x", tyA, App (Var "g", App (Var "f", Var "x")))));;
let ty2 = infer_type [] tm2;;
let test_infer_type1 = string_of_ty ty2;;
let test_infer_type2 = infer_type [] (Abs ("f", TVar "A", Var "x"));;
let test_infer_type3 = infer_type [] (Abs ("f", TVar "A", Abs ("x", TVar "B", App (Var "f", Var "x"))));;
let test_infer_type4 = infer_type [] (Abs ("f", Imp(TVar "A", TVar "B"), Abs ("x", TVar "B", App (Var "f", Var"x"))));;
let test_infer_type5 = string_of_ty (infer_type [] (Abs ("t", And (TVar "A", TVar "B"), Fst (Var "t"))));;
...
let test_check_type1 = check_type [] (Abs ("x", TVar "A", Var "x")) (Imp (TVar "A", TVar "A"));;
let test_check_type2 = check_type [] (Abs ("x", TVar "A", Var "x")) (Imp (TVar "B", TVar "B"));;
let test_check_type3 = check_type [] (Var "x") (TVar "A");;
...
let test_check_type'1 = check_type' [] (Abs ("x", TVar "A", Var "x")) (Imp (TVar "A", TVar "A"));;
let test_check_type'2 = check_type' [] (Abs ("x", TVar "A", Var "x")) (Imp (TVar "B", TVar "B"));;
let test_check_type'3 = check_type' [] (Var "x") (TVar "A");;

(** 1.8 Conjunction. *)
let and_comm = Abs ("t", And (TVar "A", TVar "B"), Pair (Fst (Var "t"), Snd (Var "t")));;
print_endline (string_of_ty (infer_type [] and_comm));;

(** 1.9 Truth. *)
let test_truth = Abs ("f", Imp (True, TVar "A"), App(Var "f", Unit));;
print_endline (string_of_ty (infer_type [] test_truth));;

(** 1.10 Disjunction. *)
let or_comm = Abs ("t", Or(TVar "A",  TVar "B"),
                          Case (Var "t",
                                "x", Right (TVar "B", Var "x"),
                                "y", Left (Var "y", TVar "A")));;
print_endline (string_of_ty (infer_type [] or_comm));;

(** 1.11 Falsity. *)
let test_falsity = Abs ("t", And (TVar "A", Imp(TVar "A", False)), Absurd (App (Snd (Var "t"), Fst (Var "t")), TVar "B"));;
print_endline (string_of_ty (infer_type [] test_falsity));;
...
let add = Abs ("m", Nat, Abs ("n", Nat, Rec (Var "m", Var "n", Abs ("x", Nat, Abs ("y", Nat, S (Var "y"))))));;
let mult = Abs ("m", Nat, Abs ("n", Nat, Rec (Var "m", Z, Abs ("x", Nat, Abs ("y", Nat, App (App (add, Var "n"), Var "y"))))));;

let test_normalize_add = normalize (App (App (add, S(S(S(S(S(Z)))))), S(S(S(Z)))));;
let test_normalize_mult = normalize (App (App (mult, S(S(S(Z)))), S(S(Z))));;
